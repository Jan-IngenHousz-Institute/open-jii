...
const zOutputEntry = z.union([
  ...
  z.object({
    v: z.unknown(),
    deviceResults: z.array(z.object({ deviceId: z.string() }).passthrough()).optional(),
...
const zSnapshotState = z.object({
  ...
  inFlight: z.null(),
  // Session-bound runtime context is never resumed: the host re-syncs its
  // roster after restore and an interrupted dispatch re-runs from its branch.
  devices: z
    .array(
      z.object({
        id: z.string(),
...
export function toSnapshot(state: RunnerState, savedAt: number): WorkbookSnapshot {
  ...
  if (state.dispatch !== null) {
    // A mid-dispatch kill re-arms at the branch; the queue is never resumed
    // (its device-connection ids would be stale anyway).
    frozen.dispatch = null;
    const prev = frozen.cellRuns[state.dispatch.branchCellId];
    frozen.cellRuns[state.dispatch.branchCellId] = {
      status: "interrupted",
