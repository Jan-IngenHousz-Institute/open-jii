...
export function createSimulatedExecutor(
  ...
  return {
    ...
    async execute(input, { signal, onProgress }) {
      ...
      if (signal.aborted) throw new Error("Command cancelled");
      const data = options.respond ? options.respond(input, call) : { echoed: input.command };
      return [{ deviceId: "sim-device", deviceLabel: "Simulator", data }];
    },
  };
}
...
interface PendingCommand {
  ...
  resolve: (value: DeviceOutcome[]) => void;
...
export function createManualExecutor(): ManualExecutor {
  ...
  return {
    ...
    settle(value) {
      ...
      next.resolve([{ deviceId: "sim-device", deviceLabel: "Simulator", data: value }]);
    },
  };
}
