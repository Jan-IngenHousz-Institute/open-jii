...
export interface OrganizationJoinRequestRejectedProps {
...
export const OrganizationJoinRequestRejected = ({
  organizationName,
  senderName = "openJII",
  host,
  baseUrl,
}: OrganizationJoinRequestRejectedProps) => {
  return (
    <EmailLayout
    ...
    >
      <Section className="rounded-t-xl bg-white px-8 pb-4 pt-8">
        ...
        <Text className="mb-6 mt-0 text-center text-[16px] leading-relaxed text-gray-600">
          Your request to join <strong>{organizationName}</strong> was not accepted at this time.
        </Text>
        <Text className="mb-0 mt-0 text-center text-[14px] leading-relaxed text-gray-500">
          You can continue to explore other organizations on openJII.
        </Text>
      </Section>
    </EmailLayout>
  );
...
export default OrganizationJoinRequestRejected;
