...
-- ============================================================================
-- Phase 2 data backfill: give every existing resource an owning organization
-- and per-resource grants, so access runs through the new authorization
-- substrate. Folded into this migration (rather than a hand-run script) so it
-- happens automatically and atomically on db:migrate in every environment.
--
...
-- visibility column default ('public') — no data statement needed. A resource
-- whose creator has no personal org (e.g. a user soft-deleted before 0035)
-- stays organization_id NULL; can() handles that safely and the creator grant
-- below still records their access.
-- ============================================================================
...
-- Experiments keep their own RBAC: mirror every experiment_members row into
-- resource_grants (role preserved) so the grant model reflects existing access.
