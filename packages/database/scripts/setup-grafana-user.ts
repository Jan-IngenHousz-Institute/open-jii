...
export async function setupGrafanaUser(): Promise<void> {
    ...
    const sql = postgres(`postgres://${masterUser}:${encodeURIComponent(masterPass)}@${host}:${port}/${name}?sslmode=${sslmode}`, {
        max: 1,
    });
    ...
    try {
        ...
        console.log(`Grafana user ${pgIdent(username)} is ready with SELECT permissions on all public tables.`);
    } finally {
        await sql.end();
    }
}

// Run standalone: node dist/scripts/setup-grafana-user.js
