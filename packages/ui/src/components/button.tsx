...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-primary hover:bg-primary-light disabled:hover:bg-primary active:bg-primary-dark text-primary-foreground shadow-xs",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, isLoading = false, children, ...props }, ref) => {
    ...
    const cursorState = isLoading ? "loading" : "disabled";

    return (
      <Comp
        className={cn(buttonVariants({ variant, size, cursorState, className }))}
        ref={ref}
