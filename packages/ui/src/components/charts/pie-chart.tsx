      ...
      data.map(
        (series) =>
          ({
            ...
            marker: {
              colors:
                series.marker?.colors || (Array.isArray(series.color) ? series.color : undefined),
...
export function DonutChart({ data, hole = 0.4, ...props }: DonutChartProps) {
  const withHole = useMemo(() => data.map((series) => ({ ...series, hole })), [data, hole]);

  return <PieChart data={withHole} {...props} />;
}
