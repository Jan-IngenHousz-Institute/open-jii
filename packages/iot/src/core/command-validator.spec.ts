...
describe("validateCommandArtifact - family trust", () => {
  it("rejects an artifact family that contradicts the host-resolved family", () => {
    const r = validateCommandArtifact(cmd("anything-goes", { family: "generic" }), {
      family: "multispeq",
    });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason).toMatch(/does not match device family/);
  });

  it("the host family wins when the artifact omits family", () => {
    const r = validateCommandArtifact(cmd("anything-goes"), { family: "generic" });
    expect(r).toEqual({ ok: true, command: "anything-goes", family: "generic" });
  });

  it("a matching artifact family passes through", () => {
    const r = validateCommandArtifact(cmd("battery", { family: "multispeq" }), {
      family: "multispeq",
    });
    expect(r.ok).toBe(true);
  });

  it("an artifact cannot escape the multispeq whitelist by claiming generic", () => {
    const r = validateCommandArtifact(cmd("rm -rf /", { family: "generic" }), {
      family: "multispeq",
    });
    expect(r.ok).toBe(false);
  });
