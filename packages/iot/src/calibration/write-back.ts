...
/** The boot dump prefix carrying the stored coefficients. */
const AMBIT_CALIBRATION_LINE = "Calibration:";
...
function ambitBootDumpReadback(
  ...
  return {
    ...
    parse: (reply) => {
      const dump = replyText(reply);
      // isValid only promises the dump reached its version line. A dump that stopped
      // before the calibration line parses to zeros, which would be read as a device
      // holding zero rather than as a coefficient nobody could read.
      if (!dump.includes(AMBIT_CALIBRATION_LINE)) {
        return null;
      }

      const info = parseAmbitBootDump(dump);
      return info.isValid ? select(info) : null;
    },
  };
}
