 ...
 * (Jan-IngenHousz-Institute/ambit, the tree the factory bench flashes) and from the
...
/** Closes an `arrun1`, which runs the array and then says so. */
...
export const AMBIT_COMMAND_OVERRIDES: Record<string, AmbitCommandOverride> = {
  ...
  // Runs the array and prints one line when it has finished, so the wait ends on that
