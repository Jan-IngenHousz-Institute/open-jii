...
4. Run `pnpm local:login` when the local API needs a signed-in call. It writes
   `.claude/session.header`; use it with `curl -H @.claude/session.header ...` and never read the
   file itself.
