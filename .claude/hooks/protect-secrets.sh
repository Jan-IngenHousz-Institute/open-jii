...
# The tool that needs a secret reads it itself (each app loads its own .env; the devkit reads its own
# tooling/devkit/.env; curl reads the session header file with -H @file). A person runs
...
# Drafting CMS content is preparation; making it live is the same handoff as shipping a build.
if mentions "release:cms[[:space:]]+publish([[:space:]]|\$)"; then
  block "this publishes CMS content to the live site; draft it and let a person publish."
fi

# The keychain, the clipboard and the environment are the other places a credential sits. Nothing
# here stores anything in the keychain, but the developer's other logins do; the one clipboard use
# is piping a pasted key into pnpm linear:auth; the session cookie travels as a file, to localhost.
...
# CMA is Contentful's management token under its short name, which matches no keyword below.
SECRET_VAR='([A-Z0-9_]*(API_KEY|ACCESS_KEY|SECRET|TOKEN|PASSWORD|PASSWD|PRIVATE_KEY|CREDENTIALS|DSN)[A-Z0-9_]*|DATABASE_URL|DB_URL|REDIS_URL|CMA)'
...
if mentions "$KEYCHAIN"; then
  block "this reads or writes the OS keychain, where the developer's other logins live."
fi
