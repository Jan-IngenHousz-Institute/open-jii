...
describe("protect-secrets hook", () => {
  const blocked: [string, unknown][] = [
    bash("cat .claude/.env"),
    bash("cat tooling/devkit/.env"),
    bash("grep LINEAR tooling/devkit/.env"),
    bash("cat .env"),
    bash("cat *.env"),
    ...
    bash("python3 -c \"open('/Users/dev/.aws/credentials')\""),
    ...
    bash("pnpm release:cms publish --entry-id 5KsDBWseXY6Q --version 3"),
    bash("CMA=cfpat-x pnpm release:cms inspect --entry-id 5KsDBWseXY6Q"),
    bash("security find-generic-password -a dev -s openjii-linear -w"),
    bash("security find-generic-password -s openjii-linear"),
    bash("security dump-keychain -d"),
    bash("security add-generic-password -a dev -s openjii-linear -w lin_api_x -U"),
    ...
    read("/Users/dev/.ssh/id_rsa"),
    read("/Users/dev/.config/gh/hosts.yml"),
    read("/home/dev/.aws/credentials"),
    grep("/repo/.claude/.env"),
    grep("/Users/dev/.ssh/id_rsa"),
    grep("/repo", ".env*"),
    grep("/repo/apps", "**/*.pem"),
  ];
  ...
  const allowed: [string, unknown][] = [
    ...
    bash("pnpm release:cms preflight"),
    bash("pnpm release:cms draft --file /tmp/note.json"),
    bash("pnpm release:cms inspect --entry-id 5KsDBWseXY6Q"),
    bash("gh pr diff 2074"),
    ...
    bash("pnpm export"),
    bash("email export"),
    bash("pnpm --filter mobile expo export"),
    bash("echo 'skipping (MACRO_SB_TEST_KEY not set)'"),
    bash('sh -c \'if [ -z "$MACRO_SB_TEST_KEY" ]; then echo "not set"; else pnpm test:run; fi\''),
    bash("echo $HOME"),
    ...
    read("/Users/dev/.claude/projects/x/memory/MEMORY.md"),
    grep("/repo/apps/web"),
    grep("/repo", "**/*.ts"),
