...
This document records what the research-assistant spike proves, what remains, and what must happen before the work can move beyond a local proof of concept. It covers OJD-1887 through OJD-1899 against base commit `8be3155b38771db95288f87081e44800dede7ebb` and the working tree reviewed on 21–22 September 2026.
...
Provider-free Python tests pass 49 cases. Focused legacy source-link tests pass 22 cases. The final web run reports 906 files and 6,486 tests passed, with 2 skipped. A fresh documentation production build and all three checks passed across 87 generated HTML pages. These results do not prove live provider availability, backend permissions, scientific quality or a production deployment.
...
| Confirmed setup, OJD-1891                        | Draft edit, discard and confirm flows call existing creation use cases. Protocol, macro and workbook creation have browser evidence. A fresh collection confirmation attached the workbook through the canonical experiment use case. Confirming drafts with a recorded entity expose Retry after refetch, with no edit, discard or premature Open action. | Finish the safe organization picker and make the create-to-record boundary crash-safe. The accepted concurrent attachment/version race remains a later hardening item.                     |
...
The earlier experiment `38f1622e...` remains a useful regression fixture: Overview shows a workbook while Design reports none because `workbookVersionId` is null. A new experiment, `84cab6e4-db03-487a-bf68-c7afcff27b65`, proves that the canonical attachment path now succeeds. Backend recovery now avoids irrelevant permission rechecks and records recovered confirmation usage; the draft card exposes recovery after refetch only when a created entity is recorded. Keep the broken fixture unchanged. Authoring remains partial because a crash between entity creation and recording its ID is not recoverable, and concurrent recoveries can still race at the domain version boundary.
...
4. Keep corpus mutation and document upload local-only until durable state and governed deletion exist.
5. Require the remote assistant job to pass after the scoped uv `0.9.28` pin. Record final focused checks and browser evidence in the PR, and keep every Linear relation as `Contributes to`.
