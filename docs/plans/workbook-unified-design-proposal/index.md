...
The loops plan splits "macro" into **Expression** (inline, per-iteration, feeds a command/branch) and **Processing step** (terminal, once per run, fan-in over collected leaves, also runs authoritatively in the platform). Verdict: **the split is correct and survives parallel branching; its cardinality rule needs one generalization lever.**

- *Why it's right:* the two roles have opposite dataflow (into the run vs out of it), opposite cardinality, and opposite run sites — and the split is precisely what keeps the resolver free of cross-iteration state (loops plan decision 2, verified against `command-payload.ts`'s purity). It also maps onto the sandbox reality: Expressions ride the existing per-item fan-out contract; Processing needs the new fan-in mode either way.
- *Under parallel branching it holds:* an Expression is **track-local** — it reads the current iteration/lane's upstream outputs through the same resolver, and lane isolation (§3.4) means it never sees a sibling lane. The Processing step reads **lane-tagged leaves** — sparse by design, exactly as branch-skipped iterations already are. Two devices on different paths produce differently-shaped leaves; that is the Processing script's job to handle, and the manifest tells it exactly what was realized.
- *What doesn't hold forever:* "exactly one per run, terminal." Method hackers running per-lane sweeps will want **per-lane/per-loop summaries** ("slope per device") before the terminal aggregate. Recommendation: keep exactly-one-terminal in v1 (as ticketed), but give the Processing cell a `scope` field (`"run"` now, a container id later) in the schema from day one, so per-scope aggregation is additive rather than a new cell type. The corresponding platform lift (per-scope `session_results` rows keyed by `(session, scope_path)`) is naturally expressed in the same MERGE pattern. [A]
- *One more honest cost:* the producer-kind reclassification (critique D6 → rev 3) must introduce **both** `"expression"` and `"processing"` wire kinds beside legacy `"macro"`, not just one; ticket 02's scope already implies this but should say it.
...
6. **Protocol→command unification (#1780, unmerged PR #1778):** landing it before Phase 2 would let the editor and the container schema speak "command" only; landing it after means the converters carry both vocabularies through the container work. Sequencing preference: before Phase 2. [A]
