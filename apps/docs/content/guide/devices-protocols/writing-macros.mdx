...
The web Workbook runner, backend single and Databricks batch execution paths,
and mobile JavaScript and Python runners apply the same compatibility
projection:
...
| `{ sample: [] }` or `[]`          | An `empty-envelope` failure; macro code is not run |
| An envelope with multiple entries | The first entry                                    |

For a multi-entry envelope, the host discards later entries and emits an
...
Projection is shallow and happens exactly once. After that single projection,
any nested envelope object is passed to macro code unchanged; it is not
projected recursively. Raw protocol and output values remain unchanged in
storage and transport; the projection applies when the host exposes a value as
`json`, as an entry in `ctx`, or to a workbook branch condition.
