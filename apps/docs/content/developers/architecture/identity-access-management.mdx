...
**Signing in accepts nothing.** A pending _organization_ invitation survives every
sign-in and sign-up untouched: it is claimed only when the recipient opens
`/platform/accept-invitation/[id]` and accepts, through Better Auth's own
...
Signing in _does_ accept pending **resource** invitations — a share of one
...
it is claimed on `/platform/accept-invitation/[id]` through Better Auth's own
...
The same exact-path mechanism still carries the _resource_ invitation auto-accept in
...
An organization cannot be deleted while it owns any resource of the six owned
types, and deletion never cascades — where GitHub cascade-deletes an organization's
...
monotonic should not vaporize public resources behind one confirmation dialog. A
device group holds no work at all, but its grants are polymorphic and cleaned only
by the group's own delete path, so cascading one away would leave access behind
naming a group that no longer exists. What the six share is that dropping a row by
SQL leaves something behind.
