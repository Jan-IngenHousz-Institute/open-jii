...
trigger intervals: five seconds for streaming queries, including Kinesis ingestion;
one minute for complete-refresh queries using only Delta inputs; and ten minutes for
...
workers; dev runs on one fixed worker. Processing time can exceed the trigger
interval, so this is not a five-second end-to-end latency guarantee.
