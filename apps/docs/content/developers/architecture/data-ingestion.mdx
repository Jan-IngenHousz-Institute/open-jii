...
{/* verified: code@1b1d99e99 2026-09-23 */}
...
complete-refresh queries with non-Delta inputs. The device aggregate retains its
explicit ten-minute interval. Production compute autoscales between two and six
workers. These intervals apply only to continuous mode; dev retains its triggered
schedule and one fixed worker. Processing time can exceed the trigger interval, so
this is not a five-second end-to-end latency guarantee.
