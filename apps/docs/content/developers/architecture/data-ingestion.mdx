...
Macro execution reads one canonical measurement value without rewriting the
stored event. The compatibility projection recognizes three top-level forms:
`{ sample: [measurement] }`, `{ sample: measurement }` when `measurement` is a
JSON object, and `[measurement]`. It selects the measurement exactly once.
Direct objects and scalars pass through unchanged.

The same rule is applied at each execution surface:

| Surface                          | Where projection occurs                                                                  |
| -------------------------------- | ---------------------------------------------------------------------------------------- |
| Workbook web                     | Backend single-item execution before the language Lambda                                 |
| Databricks Gold macro processing | Backend batch execution after the pandas UDF submits raw `data`; each item is normalized |
| Mobile                           | The local JavaScript or Pyodide Python runner before user code                           |

An empty recognized envelope (`[]` or `{ sample: [] }`) fails before macro code
runs. In a backend batch, only that item fails and valid siblings continue. If
an envelope contains multiple entries, the first is selected and the host emits
a content-free warning containing the source and counts, not measurement
values. The operation is deliberately shallow, so a nested envelope is not
...
The backend marks Lambda events as `canonical-measurement-v1`. In this change,
the JavaScript, Python, and R sandbox guards remain in **shadow mode**: they
classify boundary input and emit content-free telemetry, but do not reject or
reshape it. Enforce mode is not enabled, and the existing language wrappers are
not cleaned up or changed to enforce this contract. Macro authors should
therefore target the canonical direct-value contract and must not depend on
wrapper behavior such as reading `json.sample[0]`.
