...
> Historical record: this accepted ADR is preserved without changing its substance. Its per-experiment pipeline mechanism has been superseded by the central `centrum` implementation documented in [Data ingestion](/developers/architecture/data-ingestion).
