...
For each measurement, build the topic from the experiment's `topicPrefix` plus three segments the device chooses:

```
<topicPrefix>/<sensorVersion>/<sensorId>/<protocolId>
```
...
- `protocolId`: the `protocolId` of the procedure that produced the measurement.
...
## Receiving the file over a direct connection (SET_CONFIG)

Push-capable families receive the file over serial or BLE as one command frame:

```json
{ "command": "SET_CONFIG", "payload": { "config": { ...the file... }, "id": "<thingName>" } }
```

Persist the payload's `config` and treat it as the new full state. MultispeQ, Ambit, and MiniPAR instruments do not accept a stored configuration (their procedure travels inline per measurement); for them, and for any custom build, the downloaded file is the transport.
  ...
  for (JsonObjectConst procedure : experiment["procedures"].as<JsonArrayConst>()) {
    ...
    if (strcmp(type, "protocol") == 0) {
      ...
      String topic = String(prefix) + "/v1/" + WiFi.macAddress() + "/" +
                     procedure["protocolId"].as<const char*>();
      String body;
