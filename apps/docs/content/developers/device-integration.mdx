...
The Onboarding tab delivers a single JSON document: downloaded, pushed over a direct serial or Bluetooth connection, or published to the device's own MQTT topic. It is the device's full desired state: everything it should serve, nothing it should not. Re-issuing (onboarding with no new experiments selected) always returns the complete current state, so applying the newest file is always safe.
...
- `issuedAt`: when the platform issued this state. Echo it when acknowledging a configuration received over MQTT (below).
- `docsUrl`: this page.
...
### Over MQTT, without a visit

A connected device does not need its next configuration carried to it by hand. The platform publishes each device's current state to its own topic, retained at QoS 1:

```
device/config/v1/<thingName>
```

Subscribe after connecting. Because the message is retained, the broker replays the latest one the moment you subscribe, so a device that was offline when the configuration changed receives it on reconnect instead of missing it. The payload is the same document as the downloaded file, so a device that already reads the file needs no second parser. Treat every message as the full desired state, exactly like a re-issued file.

The platform republishes whenever that state changes: onboarding to a new experiment, re-issuing, or detaching an experiment. Deleting the device clears the topic.

After applying or refusing a configuration, publish one acknowledgement:

```
device/config/v1/<thingName>/ack
```

```json
{ "issuedAt": "2026-08-28T09:00:00.000Z", "status": "applied" }
```

Echo `issuedAt` from the configuration you acted on and set `status` to `applied` or `rejected`, adding `detail` with a reason when rejecting. This is how the platform learns which configuration each device is actually running; without it, it only knows what it sent.

Both topics are bound to the device's own thing name by its certificate policy, so a device can neither read another device's configuration nor acknowledge on its behalf.
...
Firmware for JII-managed families (ambyte, ambit, miniPAR) is delivered with
[AWS IoT Jobs](https://docs.aws.amazon.com/iot/latest/developerguide/key-concepts-jobs.html).
JII starts a rollout from a reviewed workflow; a device is never updated from the platform UI, and the device does not poll for releases.

### What the device subscribes to

The [Jobs device MQTT API](https://docs.aws.amazon.com/iot/latest/developerguide/jobs-mqtt-api.html) uses AWS's reserved topics, all scoped to the device's own thing name:
...
1. Verify `sha256` against the downloaded image before applying it; a mismatch is a `FAILED` report, not an install.
2. Report `IN_PROGRESS` when the download starts, then `SUCCEEDED` or `FAILED` with a short `statusDetails` reason.
3. Report `SUCCEEDED` without re-flashing when `version` already matches the running firmware, using the comparison rule above.
4. Keep a rollback path: apply into a spare slot and self-confirm after reboot, so a bad image cannot strand the device.
