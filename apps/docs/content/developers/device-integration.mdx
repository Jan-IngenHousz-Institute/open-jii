...
The device certificate carries a policy granting exactly these topics for its own thing, so no extra credentials are involved. A job queued while a device is offline is delivered when it next connects.
