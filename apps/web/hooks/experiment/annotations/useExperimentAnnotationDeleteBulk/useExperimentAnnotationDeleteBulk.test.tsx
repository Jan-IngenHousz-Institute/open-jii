...
describe("useExperimentAnnotationDeleteBulk", () => {
  ...
  it("should handle optimistic updates with onMutate and onError", async () => {
    ...
    const mockVariables = {
      params: { id: "exp-123" },
      body: { tableName: "table1", rowIds: ["row1", "row2"], type: "comment" },
    };
  ...
  it("should execute onMutate callback with experiment data for delete bulk", async () => {
    const mockCancelQueries = vi.fn().mockResolvedValue(undefined);
  ...
  it("should execute onMutate callback optimistic update function with valid data for delete bulk", async () => {
    const mockCancelQueries = vi.fn().mockResolvedValue(undefined);
    const mockGetQueriesData = vi.fn().mockReturnValue([]);
      ...
      body: [
        {
          data: {
            ...
            data: {
              rows: [
                { id: "row1", annotations: '[{"id":"ann-1","text":"test","type":"comment"}]' },
                { id: "row2", annotations: '[{"id":"ann-2","text":"test2","type":"comment"}]' },
              ],
            },
          },
        },
  ...
  it("should return original data in onMutate callback when no experiment data for delete bulk", async () => {
    const mockCancelQueries = vi.fn().mockResolvedValue(undefined);
    const mockGetQueriesData = vi.fn().mockReturnValue([]);
    let setQueriesDataCallback: ((oldData: unknown) => unknown) | undefined;
    const mockSetQueriesData = vi.fn().mockImplementation((_queryKey, callback) => {
  ...
  it("should execute onError callback with context and rollback for delete bulk", async () => {
    const mockSetQueryData = vi.fn();
    const mockInvalidateQueries = vi.fn().mockResolvedValue(undefined);
