...
import { DataTableAnnotationsCell } from "./data-table-annotations-cell";
...
describe("DataTableAnnotationsCell", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("should render comment and flag badges", () => {
    render(<DataTableAnnotationsCell {...mockProps} />);
  ...
  it("should render empty state when no annotations", () => {
    render(<DataTableAnnotationsCell {...mockProps} data="[]" />);

    expect(screen.getByText(/common.add/)).toBeInTheDocument();
  });

  it("should handle malformed JSON gracefully", () => {
    render(<DataTableAnnotationsCell {...mockProps} data="invalid json" />);
  ...
  it("should handle annotations without createdByName", async () => {
    ...
    render(
      <DataTableAnnotationsCell {...mockProps} data={JSON.stringify(annotationsWithoutName)} />,
    );
  ...
  it("should work without optional callback props", () => {
    ...
    expect(() => render(<DataTableAnnotationsCell {...propsWithoutCallbacks} />)).not.toThrow();
  ...
  it("should skip annotations with invalid types", () => {
    ...
    render(<DataTableAnnotationsCell {...mockProps} data={JSON.stringify(mixedAnnotations)} />);
  ...
  it("should show empty state when all annotations have invalid types", () => {
    ...
    render(<DataTableAnnotationsCell {...mockProps} data={JSON.stringify(invalidAnnotations)} />);
    expect(screen.getByText(/common.add/)).toBeInTheDocument();
  });
  ...
  describe("without contribute rights", () => {
    ...
    it("shows the annotations but no add or remove controls", async () => {
      const user = userEvent.setup();
      render(<DataTableAnnotationsCell data={mockProps.data} rowId="row-123" />);
    ...
    it("offers nothing at all on a row with no annotations", () => {
      render(<DataTableAnnotationsCell data="[]" rowId="row-123" />);
