...
describe("<ResourceCollaborators />", () => {
  ...
  it("shows the add action and the empty state for a sharer with no collaborators", async () => {
  ...
  it("keeps adding collaborators behind the modal — no search field on the panel itself", async () => {
    server.mount(contract.sharing.listGrants, { body: [] });
    ...
    // The one search surface lives inside the dialog, which is closed.
    expect(screen.queryByLabelText("sharing.granteeSearchLabel")).not.toBeInTheDocument();

    await userEvent.setup().click(screen.getByRole("button", { name: /sharing.addCollaborator/ }));
  ...
  it("adds a collaborator through the modal and shows them in the list", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /sharing.addCollaborator/ }));
  ...
  it("uses the heading overrides the host page supplies", async () => {
    server.mount(contract.sharing.listGrants, { body: [] });

    renderCollaborators({ title: "Shared access", description: "Who else can open this" });

    await waitFor(() => expect(screen.getByText("Shared access")).toBeInTheDocument());
    expect(screen.getByText("Who else can open this")).toBeInTheDocument();
    expect(screen.queryByText("sharing.cardTitle")).not.toBeInTheDocument();
  });
