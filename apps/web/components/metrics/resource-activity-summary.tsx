...
interface ResourceActivitySummaryProps {
  kind: ResourceKind;
}
 ...
 */
export function ResourceActivitySummary({ kind }: ResourceActivitySummaryProps) {
  ...
  const renderStat = (stat: Stat) => (
    <div key={stat.key} className="flex flex-col">
      ...
      <span className="text-muted-foreground text-xs">
        {t(`resourceActivity.${kind}.${stat.key}`, { days: data.windowDays })}
      </span>
    </div>
  );
