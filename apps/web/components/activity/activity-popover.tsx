...
/**
 * The activity hub trigger and popover. `icon` is the topbar-style ghost icon
 * button; `row` is a full-width sidebar row (bell + label) for the secondary
 * navigation group. Both share the same popover and unread indicator.
 */
export function ActivityPopover({
  ...
  const showIndicator = unreadCount > 0 || hasInvitations;
  ...
  const ariaLabel = `Activity${unreadCount > 0 ? ` (${unreadCount} unread)` : ""}`;

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {variant === "row" ? (
          <button type="button" aria-label={ariaLabel} className={sidebarUtilityRow(className)}>
            <Bell className="size-4 shrink-0" />
            <span className="flex-1 truncate text-left">Activity</span>
            {showIndicator && (
              <span
                aria-hidden="true"
      ...
      <PopoverContent
        ...
        side={variant === "row" ? (isMobile ? "top" : "right") : "bottom"}
        sideOffset={6}
        collisionPadding={8}
