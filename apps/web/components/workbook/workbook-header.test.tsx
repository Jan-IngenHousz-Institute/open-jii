...
describe("WorkbookHeader", () => {
  ...
  it("calls onRunAll when user clicks Run all", async () => {
    ...
    const runAllButton = screen.getByRole("button", { name: "iot.workbookBar.runAll" });
  ...
  it("hides Run all and Stop in readOnly mode", () => {
    ...
    expect(screen.queryByRole("button", { name: "iot.workbookBar.stop" })).not.toBeInTheDocument();
  ...
  it("shows Stop button instead of Run all when running", async () => {
    ...
    const stopButton = screen.getByRole("button", { name: "iot.workbookBar.stop" });
  ...
  it("calls onClearOutputs when user clicks Clear all", async () => {
    ...
    const clearButton = screen.getByRole("button", { name: "iot.workbookBar.clearAll" });
...
describe("WorkbookHeader — export menu", () => {
  ...
  it("exports the full workbook as JSON when 'Export as JSON' is clicked", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    await user.click(screen.getByRole("menuitem", { name: "iot.workbookBar.exportJson" }));
  ...
  it("exports each protocol as its own JSON file containing the raw protocol code", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    await user.click(screen.getByRole("menuitem", { name: "iot.workbookBar.exportProtocol" }));
  ...
  it("exports each macro as its own source file named after the macro with a language extension", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    await user.click(screen.getByRole("menuitem", { name: "iot.workbookBar.exportMacro" }));
  ...
  it("downloads the full workbook as a .jii file via 'Download Workbook'", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    await user.click(screen.getByRole("menuitem", { name: "iot.workbookBar.downloadWorkbook" }));
  ...
  it("disables 'Export Protocol Only' when the workbook has no protocol cells", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    expect(
      screen.getByRole("menuitem", { name: "iot.workbookBar.exportProtocol" }),
  ...
  it("disables 'Export Macro Only' when the workbook has no macro cells", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "iot.workbookBar.export" }));
    expect(screen.getByRole("menuitem", { name: "iot.workbookBar.exportMacro" })).toHaveAttribute(
      "aria-disabled",
      "true",
    );
  });
