...
describe("IotDeviceTableRow", () => {
  it("links the device name and shows its status", () => {
    ...
    expect(screen.getByText("devices.status.active")).toBeInTheDocument();
  });
  ...
  it("deletes the device and toasts on confirm", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "devices.actions.more" }));
    await user.click(await screen.findByRole("menuitem", { name: "devices.actions.delete" }));
    ...
    await waitFor(() =>
      expect(toast).toHaveBeenCalledWith(
        expect.objectContaining({ title: "devices.remove.success" }),
      ),
    );
  });
});
