...
import { IotDeviceStatusBadge, deviceRung } from "./iot-device-status-badge";
...
describe("IotDeviceStatusBadge", () => {
  it("never shows the stored word active, only the rung it resolves to", () => {
    render(<IotDeviceStatusBadge status="active" boundExperimentCount={0} />);
  ...
  it("shows onboarded once a binding exists", () => {
    render(<IotDeviceStatusBadge status="active" boundExperimentCount={2} />);
