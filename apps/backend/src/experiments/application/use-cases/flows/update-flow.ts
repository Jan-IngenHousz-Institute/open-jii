    ...
    return experimentResult.chain(async (experiment: ExperimentDto | null) => {
      ...
      if (flowGraphHasDynamicCommandRef(flowGraph)) {
        if (!isDynamicCommandPublishEnabled()) {
          this.logger.warn({
            msg: "Blocked flow update: dynamic command publication is disabled",
        ...
        if (issues.length > 0) {
          this.logger.warn({
            msg: "Blocked flow update: dynamic command references are structurally invalid",
            operation: "updateFlow",
            experimentId,
      ...
      return existing.chain(async (flow) => {
        ...
        return this.flowRepository.update(experimentId, flowGraph);
      });
    });
  }
}
