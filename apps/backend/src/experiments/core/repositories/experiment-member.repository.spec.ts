...
describe("ExperimentMemberRepository", () => {
  ...
  describe("isAdmin", () => {
    it("should return true if the user is an admin of the experiment", async () => {
      const { experiment } = await testApp.createExperiment({
        name: "Admin Check Experiment",
        userId: testUserId,
      });
      const adminId = await testApp.createTestUser({
      ...
      const result = await repository.isAdmin(experiment.id, adminId);
      assertSuccess(result);
      expect(result.value).toBe(true);
    });

    it("should return false if the user is a member but not an admin", async () => {
      const { experiment } = await testApp.createExperiment({
        name: "Non-Admin Check Experiment",
        userId: testUserId,
      });
      const memberId = await testApp.createTestUser({
      ...
      const result = await repository.isAdmin(experiment.id, memberId);
      assertSuccess(result);
      expect(result.value).toBe(false);
    });

    it("should return false if the user is not a member of the experiment", async () => {
      const { experiment } = await testApp.createExperiment({
        name: "Non-Member Admin Check",
        userId: testUserId,
      });
      const nonMemberId = await testApp.createTestUser({
        email: "not-a-member@example.com",
      });
      const result = await repository.isAdmin(experiment.id, nonMemberId);
      assertSuccess(result);
      expect(result.value).toBe(false);
    });

    it("should return false for a non-existent experiment", async () => {
      const nonExistentExperimentId = "00000000-0000-0000-0000-000000000000";
