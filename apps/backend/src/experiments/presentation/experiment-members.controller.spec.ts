...
describe("ExperimentMembersController", () => {
  ...
  describe("removeExperimentMember", () => {
    ...
    it("should return 404 when removing member from non-existent experiment", async () => {
      ...
      const path = testApp.resolveOrpcPath(contract.experiments.removeExperimentMember, {
        id: nonExistentId,
        memberId: memberId,
      });
  ...
  describe("updateExperimentMemberRole", () => {
    it("should update a member's role from member to admin", async () => {
      ...
      const path = testApp.resolveOrpcPath(contract.experiments.updateExperimentMemberRole, {
        id: experiment.id,
        memberId: memberId,
      });
  ...
  describe("transferExperimentAdmin", () => {
    const transferPath = () =>
      testApp.resolveOrpcPath(contract.experiments.transferExperimentAdmin, {});
