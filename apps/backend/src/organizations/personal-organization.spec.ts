...
describe("ensurePersonalOrganization", () => {
  ...
  describe("syncPersonalOrganizationName", () => {
    it("creates the org (named from the given name) when none exists", async () => {
      const orgId = await syncPersonalOrganizationName(testApp.database, {
        id: userId,
        name: "Grace Hopper",
      });
      ...
      const [org] = await testApp.database
        ...
        .where(eq(organizations.id, orgId));
      expect(org.slug).toBe(personalOrgSlug(userId));
      expect(org.name).toBe("Grace Hopper's workspace");
    });

    it("renames an existing org whose name was frozen at creation", async () => {
      // Simulate the signup hook creating the org before a profile exists.
      const orgId = await ensurePersonalOrganization(testApp.database, { id: userId });
      const [before] = await testApp.database
        .select()
        .from(organizations)
        .where(eq(organizations.id, orgId));
      expect(before.name).toBe("Personal workspace");

      const again = await syncPersonalOrganizationName(testApp.database, {
        id: userId,
        name: "Ada Lovelace",
      });
      expect(again).toBe(orgId);
      ...
      const [after] = await testApp.database
        ...
        .from(organizations)
        .where(eq(organizations.id, orgId));
      expect(after.name).toBe("Ada Lovelace's workspace");
    });
  });
