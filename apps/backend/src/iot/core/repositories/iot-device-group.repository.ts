...
@Injectable()
export class IotDeviceGroupRepository {
  constructor(
    @Inject("DATABASE")
  ...
  async findById(groupId: string): Promise<Result<IotDeviceGroupWithCountDto | null>> {
    return tryCatch(async () => {
  ...
  async delete(groupId: string): Promise<Result<IotDeviceGroupDto[]>> {
    return tryCatch(() =>
