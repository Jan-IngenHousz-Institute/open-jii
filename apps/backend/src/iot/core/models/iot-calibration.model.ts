...
export type ActiveCalibrationBlockDto = Omit<
  ...
  "validFrom" | "writtenToDeviceAt"
> & {
  validFrom: Date;
  writtenToDeviceAt: Date | null;
};

/** Assembled per device from the newest approval of each block, so it has no row of its own. */
export interface ActiveDeviceCalibrationDto {
  deviceId: string;
  blocks: Record<string, ActiveCalibrationBlockDto>;
}
