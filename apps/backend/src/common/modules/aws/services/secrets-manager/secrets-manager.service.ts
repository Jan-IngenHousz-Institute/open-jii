...
export class AwsSecretsManagerService {
  ...
  async getSecret(secretArn: string, forceRefresh = false): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const parsedSecret: unknown = JSON.parse(response.SecretString);
      if (
        parsedSecret === null ||
        typeof parsedSecret !== "object" ||
        Array.isArray(parsedSecret)
      ) {
        throw new Error(`Secret ${secretArn} is not a JSON object`);
      }

      const isRecord = (val: unknown): val is Record<string, unknown> => {
        if (typeof val !== "object" || val === null || Array.isArray(val)) return false;
        return Object.keys(val).every((k) => typeof k === "string");
      };
      if (!isRecord(parsedSecret)) {
        throw new Error(`Secret ${secretArn} is not a valid Record<string, unknown>`);
      }
      const secretValue: Record<string, unknown> = parsedSecret;
