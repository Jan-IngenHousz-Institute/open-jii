...
export class SecretsManager {
  ...
  async getSecret(secretArn: string, forceRefresh = false): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      if (cached) {
        console.warn(`Failed to refresh secret ${secretArn}, using cached value:`, error);
        return cached.value;
      }
   ...
   */
  async getDatabaseCredentials(secretArn: string): Promise<{ username: string; password: string }> {
