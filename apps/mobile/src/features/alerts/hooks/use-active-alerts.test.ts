...
vi.mock("~/features/alerts/services/fetch-active-alerts", () => ({
  fetchActiveAlerts: (...args: unknown[]) => mockFetchActiveAlerts(...args),
}));

vi.mock("~/shared/config/load-env-variables-from-expo", () => ({
...
describe("useActiveAlerts", () => {
  ...
  it("filters out dismissed alerts by internalName", () => {
  ...
  it("falls back to sys.id for dismissal when internalName is null", () => {
