import { getSerialPortConnection } from "~/features/connection/services/device-connection-manager/serial-port-connection";
...
export function createSerialPortCommandExecutor(deviceId: string) {
  const connection = getSerialPortConnection(deviceId);
  if (!connection) {
    return undefined;
  }
