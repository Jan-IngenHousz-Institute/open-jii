...
import { useRequestJoinExperiment } from "~/features/experiments/hooks/use-request-join-experiment";
...
interface ExperimentJoinRequestSheetProps {
  experimentId: string;
  experimentName: string;
}

export const ExperimentJoinRequestSheet = forwardRef<
  BottomSheetModal,
  ExperimentJoinRequestSheetProps
>(function ExperimentJoinRequestSheet({ experimentId, experimentName }, ref) {
  const themeColors = useThemeColors();
  const { t } = useTranslation(["common", "experiments"]);
  const insets = useSafeAreaInsets();
  const [message, setMessage] = useState("");
  const { requestJoin, isPending } = useRequestJoinExperiment(experimentName);
  ...
  const send = () => {
    ...
    requestJoin(
      { id: experimentId, message: normalizeJoinMessage(message) },
      // No onError: the hook owns the toast, and a failure must leave the sheet
      // open with the message intact so the user can retry.
      {
        onSuccess: () => {
          setMessage("");
          dismiss();
      ...
      <BottomSheetView className="bg-card px-4" style={{ paddingBottom: insets.bottom + 16 }}>
        ...
        <View className="mt-3">
          <Input
            ...
            placeholder={t("experiments:join.messagePlaceholder")}
            accessibilityLabel={t("experiments:join.sheetHint")}
            multiline
            numberOfLines={4}
          ...
          <Text className="text-muted-body mt-1 text-right text-[11px]">
            {t("experiments:join.messageCounter", {
              n: message.length,
        ...
        <View className="mt-4 gap-2">
          <Button
            title={isPending ? t("experiments:join.sending") : t("experiments:join.send")}
            onPress={send}
            isLoading={isPending}
