...
describe("time-sync", () => {
  ...
  afterEach(async () => {
    const { stopTimeSync } = await import("~/shared/time/time-sync");
    stopTimeSync();
    vi.useRealTimers();
    vi.resetModules();
  });
  ...
  it("should register a foreground listener on start", async () => {
    const { startTimeSync } = await import("~/shared/time/time-sync");
    const { onAppForeground } = await import("~/shared/device/app-lifecycle");
  ...
  it("should trigger a sync when app comes to foreground", async () => {
    const { startTimeSync, getTimeSyncState } = await import("~/shared/time/time-sync");
  ...
  it("should unsubscribe from foreground events on stop", async () => {
    const { startTimeSync, stopTimeSync } = await import("~/shared/time/time-sync");
  ...
  it("should correct for device clock drift on foreground sync", async () => {
    const { startTimeSync, getSyncedUtcNow } = await import("~/shared/time/time-sync");
  ...
  describe("restoreState from AsyncStorage", () => {
    it("should restore persisted state on startTimeSync", async () => {
      const persisted: import("~/shared/time/time-sync").TimeSyncState = {
        offsetMs: 4200,
  ...
  describe("getSyncedUtcISO", () => {
    it("returns a UTC ISO string (Z suffix) after sync", async () => {
      const { startTimeSync, getSyncedUtcISO } = await import("~/shared/time/time-sync");
  ...
  describe("ensureSynced", () => {
    it("should resolve immediately if already synced", async () => {
      const { startTimeSync, ensureSynced } = await import("~/shared/time/time-sync");
    ...
    it("should wait for sync to complete when not yet synced", async () => {
      const { ensureSynced, getTimeSyncState } = await import("~/shared/time/time-sync");
    ...
    it("should reject after timeout if sync never succeeds", async () => {
      ...
      const { ensureSynced } = await import("~/shared/time/time-sync");
      const { toast } = await import("sonner-native");
