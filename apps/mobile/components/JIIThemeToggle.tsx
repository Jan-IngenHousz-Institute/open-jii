...
export default function JIIThemeToggle({ style }: ThemeToggleProps) {
  ...
  const getButtonStyle = (themeType: 'system' | 'light' | 'dark') => {
    ...
    return {
      ...
      borderColor: isActive ? colors.primary.dark : theme.isDark ? colors.dark.border : colors.light.border,
    };
  };
  
  const getTextColor = (themeType: 'system' | 'light' | 'dark') => {
    const isActive = theme.themePreference === themeType;
    return isActive ? colors.neutral.white : theme.isDark ? colors.dark.onSurface : colors.light.onSurface;
  };
