      ...
      <View style={styles.statusRow}>
        ...
        <View style={styles.statusItem}>
          {bluetoothConnected ? (
            ...
            <BluetoothOff size={20} color={theme.isDark ? colors.dark.inactive : colors.light.inactive} />
          )}
          <Text style={[
            styles.statusText, 
            { color: theme.isDark ? colors.dark.onSurface : colors.light.onSurface },
            !bluetoothConnected && { color: theme.isDark ? colors.dark.inactive : colors.light.inactive }
          ]}>
        ...
        <View style={styles.statusItem}>
          ...
          <Text style={[
            styles.statusText, 
            { color: theme.isDark ? colors.dark.onSurface : colors.light.onSurface },
            !usbConnected && { color: theme.isDark ? colors.dark.inactive : colors.light.inactive }
          ]}>
      ...
      {(bluetoothConnected || usbConnected) && deviceName && (
        <View style={[
          ...
          <Text style={[
            ...
            Connected to: <Text style={[styles.deviceName, { color: colors.primary.dark }]}>
              {deviceName}
            </Text>
          </Text>
          <Text style={[
            ...
            via {connectionType === 'bluetooth' ? 'Bluetooth Classic' : 
                connectionType === 'ble' ? 'Bluetooth LE' : 
                connectionType === 'usb' ? 'USB Serial' : 'Unknown'}
          </Text>
        </View>
      )}
    </View>
