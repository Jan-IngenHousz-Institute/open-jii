...
function ArrayNth(arr, size, idx) {
  ...
  var sliced = [];
  for (var i = idx; i < arr.length; i = i + size) {
    sliced.push(arr[i]);
  }
  return sliced;
}
...
function ArrayRange(start, stop, step, transform) {
  if (start === undefined || typeof start != "number") return null;
  ...
  if (["none", "log", "ln", "x2"].indexOf(transform) == -1) return null;

  var arr = [];

  while (step > 0 ? stop > start : stop < start) {
    if (transform == "none") arr.push(start);
    if (transform == "log") arr.push(Math.log(start) / Math.LN10);
    if (transform == "ln") arr.push(Math.log(start));
    if (transform == "x2") arr.push(Math.pow(start, 2));
    start += step;
  }
  return arr;
}
...
function GetLabelLookup(json) {
  var lookup = {};
  // Return null if there is no (valid) set
  if (json == null || !Array.isArray(json.set)) return null;
  for (var i in json.set) {
    if (json.set[i].label !== undefined) {
      ...
      if (lookup.unknown === undefined) lookup.unknown = [];
...
function MathLINREG(x, y) {
  ...
  if (xn == yn) {
    ...
    // calculate r
    var r =
      (xySum - (1 / xn) * xSum * ySum) /
      Math.sqrt((xxSum - (1 / xn) * Math.pow(xSum, 2)) * (yySum - (1 / xn) * Math.pow(ySum, 2)));
...
function MathMEDIAN(values) {
  ...
  if (values && Array.isArray(values)) {
    ...
    if (check) return parseFloat(val);
...
function MathSTDERR(values) {
  ...
  if (values && Array.isArray(values)) {
    ...
    if (check) return parseFloat(stderr);
...
function MathSTDEV(values) {
  ...
  if (values && Array.isArray(values)) {
    ...
    if (check) return parseFloat(stdev);
...
function MathSTDEVS(values) {
  ...
  if (values && Array.isArray(values)) {
    ...
    if (check) return parseFloat(stdevs);
...
function MathVARIANCE(values) {
  ...
  if (values && Array.isArray(values)) {
    ...
    if (check || values.length == 0) return parseFloat(variance);
...
function info(msg, output) {
  if (output.messages === undefined) output.messages = {};
  if (output.messages.info === undefined) output.messages.info = [];
...
function warning(msg, output) {
  if (output.messages === undefined) output.messages = {};
  if (output.messages.warning === undefined) output.messages.warning = [];
...
function danger(msg, output) {
  if (output.messages === undefined) output.messages = {};
  if (output.messages.danger === undefined) output.messages.danger = [];
...
function NonLinearRegression(data, options) {
  ...
  var presets = {
    iterations:
      options.iterations === undefined || options.iterations > 2000 ? 200 : options.iterations,
  ...
  function STUDT(r, t) {
    ...
    var e = Math.sin(a),
      u = Math.cos(a);
    return t % 2 == 1
      ? 1 - (a + e * u * STATCOM(u * u, 2, t - 3, -1)) / (Math.PI / 2)
      : 1 - e * STATCOM(u * u, 1, t - 3, -1);
  }
  ...
  function ASTUDT(r, t) {
    for (var n = 0.5, a = 0.5, e = 0; a > 1e-15; )
      (e = 1 / n - 1), (a /= 2), STUDT(e, t) > r ? (n -= a) : (n += a);
    return e;
  }
  ...
  function Fmt(r) {
    ...
    return (
      Math.abs(r) < 5e-5 && (r = 0),
      (t = r >= 0 ? " " + (r + 5e-5) : " " + (r - 5e-5)),
      (t = t.substring(0, t.indexOf(".") + 5)),
      t.substring(t.length - 10, t.length)
    );
  }
  ...
  function vFmt(r) {
    ...
    return (
      Math.abs(r) < 5e-7 && (r = 0),
      (t = r >= 0 ? " " + (r + 5e-7) : " " + (r - 5e-7)),
      (t = t.substring(0, t.indexOf(".") + 7)),
      t.substring(t.length - 18, t.length)
    );
  }
  ...
  function Iterate(r, a, P) {
    ...
    Par[7] = h;
    var E = "Y = " + a.equation + "\n";
    for (var i = 1; i <= a.cVar; i++) E += " x" + i + " ";
    E += " Y yc Y-yc SEest YcLo YcHi \n";
    var m = 0;
    for (b = 0; b < a.cPar * (a.cPar + 1); b++) Arr[b] = 0;
    for (var i = 1; i <= a.cPts; i++) {
      for (b = 0; b < a.cVar; b++) (xArr[b] = r[i - 1][0]), (v = r[i - 1][1]);
      var g = xArr[0];
      "LN(X2)" == a.x1Trans && (g = Math.log(xArr[0])),
        "SQRT(X2)" == a.x2Trans && (g = Math.sqrt(xArr[0])),
        "1/X2" == a.x2Trans && (g = 1 / xArr[0]);
      var q = g,
        D = xArr[1];
      "LN(X2)" == a.x2Trans && (D = Math.log(xArr[1])),
        "SQRT(X2)" == a.x2Trans && (D = Math.sqrt(xArr[1])),
        "1/X2" == a.x2Trans && (D = 1 / xArr[1]);
      var X = xArr[2];
      "LN(X3)" == a.x3Trans && (X = Math.log(xArr[2])),
        "SQRT(X3)" == a.x3Trans && (X = Math.sqrt(xArr[2])),
        "1/X3" == a.x3Trans && (X = 1 / xArr[2]);
      var F = xArr[3];
      "LN(X4)" == a.x4Trans && (F = Math.log(xArr[3])),
        "SQRT(X4)" == a.x4Trans && (F = Math.sqrt(xArr[3])),
        "1/X4" == a.x4Trans && (F = 1 / xArr[3]);
      var d = xArr[4];
      "LN(X5)" == a.x5Trans && (d = Math.log(xArr[4])),
        "SQRT(X5)" == a.x5Trans && (d = Math.sqrt(xArr[4])),
        "1/X5" == a.x5Trans && (d = 1 / xArr[4]);
      var p = xArr[5];
      "LN(X6)" == a.x6Trans && (p = Math.log(xArr[5])),
        "SQRT(X6)" == a.x6Trans && (p = Math.sqrt(xArr[5])),
        "1/X6" == a.x6Trans && (p = 1 / xArr[5]);
      var R = xArr[6];
      "LN(X7)" == a.x7Trans && (R = Math.log(xArr[6])),
        "SQRT(X7)" == a.x7Trans && (R = Math.sqrt(xArr[6])),
        "1/X7" == a.x7Trans && (R = 1 / xArr[6]);
      var C = xArr[7];
      "LN(X8)" == a.x8Trans && (C = Math.log(xArr[7])),
        "SQRT(X8)" == a.x8Trans && (C = Math.sqrt(xArr[7])),
        "1/X8" == a.x8Trans && (C = 1 / xArr[7]);
      ...
      "w" == a.SEy && (U = v),
        "Rep" == a.SEy && (U = N),
        "Sqrt(Y)" == a.SEy && (U = Math.sqrt(Y)),
        0 == U && (U = 0.001);
      var u = a.yTrans.toUpperCase(),
        Q = Y;
      "LN(Y)" == u && (Q = Math.log(Y)),
        "LN(SQRT)" == u && (Q = Math.sqrt(Y)),
        "1/Y" == u && (Q = 1 / Y),
        "LN(Y)" == u && (U /= Y),
        "SQRT(Y)" == u && (U /= Q),
        "1/Y" == u && (U = (U / Y) * Y),
        ...
        a.LeastAbs &&
          (U *=
            Y < l
              ? MAX(Math.sqrt(A * Math.abs(Y - l)), 0.001 * Y)
              : MAX(Math.sqrt((1 - A) * Math.abs(Y - l)), 0.001 * Y)),
        (t += 1 / (U * U)),
        (c += l / (U * U)),
        (x += ((Y - ccAv) * (Y - ccAv)) / (U * U));
      for (var b = 0; b < a.cPar; b++) {
        ...
        (Par[b] += B),
          ...
          (f = Par[6]),
          (h = Par[7]),
          (ycIncr = Equation(a.equation, e, S, T, n, o, M, f, h, g, q)),
          (Der[b] = (ycIncr - l) / (B * U)),
          a.Centered &&
            ((Par[b] -= B),
            (e = Par[0]),
            ...
            (f = Par[6]),
            (h = Par[7]),
            (ycDecr = Equation(a.equation, e, S, T, n, o, M, f, h, g, q)),
            (Der[b] = (ycIncr - ycDecr) / (2 * B * U))),
          (Par[b] = W),
          (e = Par[0]),
          (S = Par[1]),
          (T = Par[2]),
      ...
      var V = 0;
      for (b = 0; b < a.cPar; b++)
        for (V += Cov[ix(b, b)] * Der[b] * Der[b], k = b + 1; k < a.cPar; k++)
          V += 2 * Cov[ix(b, k)] * Der[b] * Der[k];
      V = U * Math.sqrt(V);
      var _ = l,
        w = l - s * V,
        H = l + s * V;
      "LN(Y)" == u && ((_ = Math.exp(l)), (w = Math.exp(w)), (H = Math.exp(H))),
        "SQRT(Y)" == u && ((_ = l * l), (w *= w), (H *= H)),
        "1/Y" == u && ((_ = 1 / l), (w = 1 / w), (H = 1 / H)),
        (E += Fmt(yo) + Fmt(_) + Fmt(yo - _) + Fmt(V) + Fmt(w) + Fmt(H) + "\n");
    }
    (ccSW = t), (ccAv = c / ccSW), (ccSD = x / ccSW);
    var j = (ccSD - m / ccSW) / ccSD,
      z = Math.sqrt(j);
    for (
      E += "\nCorr. Coeff. = " + vFmt(z) + "; r*r = " + vFmt(j),
        RMS = Math.sqrt(m / MAX(1, dgfr)),
        E += "\nRMS Error = " + vFmt(RMS) + "; d.f = " + dgfr + "; SSq = " + vFmt(m) + "\n",
        AIC = a.cPts * Math.log(m / a.cPts) + 2 * (a.cPar + 1),
        a.cPts >= a.cPar + 2
          ? (AICc = AIC + (2 * (a.cPar + 1) * (a.cPar + 1 + 1)) / (a.cPts - (a.cPar + 1) - 1))
          : (AICc = AIC),
        E += "AIC = " + vFmt(AIC) + "; AIC(corrected) = " + vFmt(AICc) + "\n",
        i = 0;
      i < a.cPar;
      i++
    ) {
      ...
      for (b = 0; b < a.cPar; b++)
        if (i != b)
          for (G = Arr[ix(b, i)], Arr[ix(b, i)] = 0, k = 0; k <= a.cPar; k++)
            Arr[ix(b, k)] = Arr[ix(b, k)] - G * Arr[ix(i, k)];
    }
    var J = "\nParameter Estimates...\n",
      K = [];
    for (var i = 0; i < a.cPar; i++)
      (Par[i] = Par[i] + a.RelaxF * Arr[ix(i, a.cPar)]),
    ...
    for (b = 0; b < a.cPar; b++)
      for (k = b; k < a.cPar; k++)
        (Cov[ix(b, k)] = Arr[ix(b, k)] * RMS * RMS),
          (v = Arr[ix(b, k)] / Math.sqrt(Arr[ix(b, b)] * Arr[ix(k, k)])),
          (O += "B(" + (b + 1) + "," + (k + 1) + ")="),
          b != k && (O += "B(" + (k + 1) + "," + (b + 1) + ")="),
          (O += " "),
          (O += Cov[ix(b, k)]),
  ...
  function Equation(eqtn, a, b, c, d, e, f, g, h, x, t) {
    if (eqtn == "b + a * e(- x / c)") return b + a * Math.exp(-x / c);
    if (eqtn == "( a - c ) * e( - b * t ) + c") return (a - c) * Math.exp(-b * t) + c;
    if (eqtn == "( c + a / ( 1 + b / x ) )") return c + a / (1 + b / x);
    if (eqtn == "( c + a * a / ( 1 + b / x ) )") return c + (a * a) / (1 + b / x);
    return null;
  }
  ...
  for (it = 0; it < presets.iterations; it++) {
    ...
    if ((convergence <= tmp.RMS_error && it > 1) || isNaN(tmp.RMS_error)) break;
    parameters = tmp.par;
...
function TransformTrace(fn, a1, a2) {
  ...
  if (typeof a2 == "number") {
    ...
    if (fn == "abs") {
      for (var i in a1) trace.push(-Math.log(a1[i] / a1[0]) / Math.LN10);
    }
...
function MathEXPINVREG(input_raw) {
  ...
  var asymptote =
    (riemannSq - (riemann * (y[0] + y[y.length - 1])) / 2) /
...
function calcSunAngle(roll, pitch, compass, azimuth, altitude) {
  ...
  var percent =
    solarVector[0] * deviceVector[0] +
    solarVector[1] * deviceVector[1] +
    solarVector[2] * deviceVector[2];

  return {
    angle: (acos(percent) * 180) / pi,
    percent: percent,
  };
}
...
function transpose(x) {
  var i,
    ...
    A1,
    Bj;
  for (var j = 0; j < n; j++) ret[j] = new Array(m);
  for (var i = m - 1; i >= 1; i -= 2) {
    A1 = x[i];
    A0 = x[i - 1];
    for (var j = n - 1; j >= 1; --j) {
      Bj = ret[j];
...
function dotMMbig(x, y) {
  ...
  --p;
  --m;
  for (var i = m; i !== -1; --i) A[i] = new Array(n);
  --n;
