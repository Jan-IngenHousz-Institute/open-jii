...
function processGenerateDir(dirPath: string): { dataset: Dataset; entry: TestCase } | null {
  ...
  const entry: TestCase = {
    ...
    script: (() => {
      const plain = Buffer.from(readFileSync(macro.path, "utf-8")).toString("base64");
      const key = Buffer.from(process.env.MACRO_SB_TEST_KEY!, "base64");
      const iv = randomBytes(16);
      const c = createCipheriv("aes-256-cbc", key, iv);
      return (
        iv.toString("hex") +
        ":" +
        Buffer.concat([c.update(plain, "utf8"), c.final()]).toString("hex")
      );
    })(),
    items,
