...
def add_annotation_column(df, annotations_df):
    ...
    try:
        ...
        annotations_structured = annotations_df.select(
            F.col("experiment_id"),
            F.col("row_id"),
            F.struct(
                F.col("id"),
        ...
        annotations_grouped = annotations_structured.groupBy("experiment_id", "row_id").agg(
            F.sort_array(F.collect_list("annotation")).alias("db_annotations")
        )
        ...
        print(f"Warning: Could not join with annotations table: {e!s}")
        ...
        if "annotations" in df.columns:
            ...
            return df.withColumn("annotations", F.array().cast(ArrayType(annotation_schema)))
