WITH
-- Each experiment table's schemas from the pipeline, with its row count and an
-- upload's newest name taken when someone reads. A count cannot be kept
-- incrementally on classic compute, and the gold tables are clustered by
-- experiment, so a read scans only the experiment it asks for. ${catalog} is
