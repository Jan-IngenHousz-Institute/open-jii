...
locals {
  ...
  # Build policy statements for requested services
  policy_statements = [
    for service in var.services : {
      Effect   = "Allow"
      Action   = local.service_policies[service].actions
      Resource = local.service_policies[service].resource
      Sid      = "${title(replace(service, "-", ""))}Permissions"
    }
  ]
}
...
resource "aws_iam_policy" "oidc_role_policy" {
  ...
  description = "Policy for ${var.role_name} with permissions for: ${join(", ", var.services)}"
  policy = jsonencode({
    Version   = "2012-10-17"
